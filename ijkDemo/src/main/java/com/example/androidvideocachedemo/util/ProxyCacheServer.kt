...
//                val s = url.split("/").toTypedArray()
//                s[s.size - 1]
                getFileSubPathWithNameByUrl(url)
            }
            .build()
    }

    fun getFileSubPathWithNameByUrl(url: String): String {
        return Uri.parse(url).let {
            if (it.host.isNullOrBlank())
                url
            else it.host + it.path
        }
    }
