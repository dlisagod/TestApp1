...
//        String requestHost = url.getPort() > 0 ? url.getHost() + ":" + url.getPort() + url.getPath() : url.getHost();
//        String requestHost = url.getPort() > 0 ? url.getHost() + ":" + url.getPort() + url.getPath() : url.getHost();
//        String requestAddress = format("%s://%s", url.getProtocol(), requestHost);
        String requestAddress = UrlUtil.getSimpleBasePath(source.getUrl());
        //将预先下载完的内容替换ts为代理链接
    ...
    @Override
    protected void onCachePercentsAvailableChanged(int percents, boolean percentsChanged, long speedPerSecond) {
//        Log.d(getClass().getSimpleName(), "onCachePercentsAvailableChanged()" + " listener:" + listener + " percents:" + percents + " percentChanged:" + percentsChanged);
        if (listener != null) {
            listener.onCacheAvailable(cache.file, source.getUrl(), percents, percentsChanged, speedPerSecond);
        }
    }
}
