...
import com.danikula.videocache.sourcestorage.UrlUtil;
...
public class HttpProxyCacheServer {
    ...
    private ExecutorService getM3U8TSListenerExecutor = Executors.newSingleThreadScheduledExecutor();
    ...
    public void registerCacheListener(CacheListener cacheListener, String url) {
        ...
        synchronized (clientsLock) {
            try {
                ...
                if (M3u8ProxyUtil.isM3u8Url(url)) {
                    getM3U8TSListenerExecutor.execute(new M3u8TSUrlListenerRunnable(url, cacheListener));
                }
    ...
    public void shutdown() {
        ...
        getM3U8TSListenerExecutor.shutdownNow();
     ...
     * 用于给m3u8的ts文件加下载监听
     */
    private final class M3u8TSUrlListenerRunnable implements Runnable {

        private final String url;
        private final CacheListener cacheListener;

        public M3u8TSUrlListenerRunnable(String url, CacheListener cacheListener) {
            this.url = url;
...
//                List<String> mediaList = M3u8ProxyUtil.getM3u8TS(body, url);
//                List<String> mediaList = M3u8ProxyUtil.getM3u8TS(body, UrlUtil.getSimpleBasePath(source.getUrl()));
                List<String> mediaList = M3u8ProxyUtil.getM3u8TS(body, UrlUtil.getSimpleBasePath(url));
                for (String ts : mediaList) {
                    getClients(ts).registerCacheListener(cacheListener);
                    Log.d(TAG, "M3u8TSUrlListenerRunnable ts:" + ts);
                }
            } catch (Throwable e) {
                LOG.error("url:" + url, e);
            } finally {
